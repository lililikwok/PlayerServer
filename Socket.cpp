...
int CSocket::Init(const CSockParam &param)
{
    ...
    if (m_param.attr & SOCK_ISSERVER)
    {
        ...
        {
            ret = bind(m_socket, m_param.addrin(), sizeof(sockaddr_in)); // 绑定
        }
        else
        {
...
{
    // 保证套接字已经初始化完成// [套接字状态] 0:初始化未完成 1:初始化完成 2:连接完成 3:已经关闭
    ...
    if (m_param.attr & SOCK_ISSERVER) //// 需要accept() 接受连接
    {
        if (pClient == NULL)
            return -2;
        CSockParam param; // 这个是要准备接受的客户端的地址参数 默认地址类型为attr=0  客户端\阻塞\TCP\本地套接字
        int fd = -1; //这个是准备接受的客户端的套接字的文件描述符
        socklen_t len = 0;
